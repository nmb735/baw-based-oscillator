# =================================================================================================================================== #
# ----------------------------------------------------------- DESCRIPTION ----------------------------------------------------------- #
# Comparison script: re-parse the Pierce and negative-resistance Keysight ADS "List" exports (both at nominal Vcc = 3.3 V) and        #
# produce overlay figures comparing the two topologies. No transistor / resonator comparison - that data only exists for Pierce.      #
...
PIERCE_DIR     = os.path.join(_ROOT, 'data', '2026-09-22', 'simulations', 'ADS', 'Pierce-3p3V')
NEGRES_DIR     = os.path.join(_ROOT, 'data', '2026-09-22', 'simulations', 'ADS', 'Negative-Resistance-3p3V')
OUTPUT_DIR     = os.path.join(_ROOT, 'output', '2026-09-22', 'simulations', 'ADS', 'Comparison-3p3V')
LABEL_PIERCE   = 'Pierce'
...
# =================================================================================================================================== #
# ------------------------------------------------------------- DESIGN CONSTANTS ---------------------------------------------------- #
# Pierce (see main_oscillator.py for how these were determined).
PIERCE_F_MARK_OLG_GHZ     = 2.963    # Randall-Hock open-loop gain: solved zero-phase crossing.                                       #
PIERCE_F_MARK_NYQUIST_GHZ = 2.965    # OscTest Nyquist: solved 0 deg crossing.                                                        #
PIERCE_CARRIER_GHZ        = 2.976    # Oscillator carrier frequency (transient FFT peak).                                             #
PIERCE_ZO_OSCTEST_OHM     = 21.5     # OscTest reference (probe) impedance.                                                           #

# Negative resistance (see main_oscillator_negative_resistance.py for how these were determined).
NEGRES_F_MARK_OLG_GHZ     = 2.970    # Nominal marker only - Gc probe data is numerically degenerate (see that script's NOTE).        #
NEGRES_F_MARK_NYQUIST_GHZ = 2.983    # OscTest Nyquist: loop phase wraps through +/-180 deg here, not 0.                              #
NEGRES_CARRIER_GHZ        = 2.970    # Oscillator carrier frequency (transient FFT peak).                                             #
...
pierce_olg_mag     = pp('open_loop_gain_magnitude.csv')
pierce_olg_phase   = pp('open_loop_gain_phase.csv')
pierce_nq_mag      = pp('nyquist_s11_dB.csv')
pierce_nq_phase    = pp('nyquist_s11_phase.csv')
pierce_tran_time   = pp('tran_vout_time.csv')
pierce_tran_fft    = pp('tran_vout_time_fft.csv')
